...
def process_update(update, get_bot_response):
    ...
    try:
        ...
        if 'text' in message:
            ...
            safe_user_message = validate_user_input(safe_user_message)
            safe_user_message = escape_user_input(safe_user_message)

            # Get bot response
            bot_reply = get_bot_response(safe_user_message)

            # Sanitize bot reply before sending
            safe_bot_reply = sanitize_user_input(bot_reply)
            safe_bot_reply = validate_user_input(safe_bot_reply)
            safe_bot_reply = escape_user_input(safe_bot_reply)

            # Send response
            result = send_message(chat_id, safe_bot_reply)
